...
| [Secure Private Key Storage](features/secure-key-storage.md) | Keys stored encrypted in the DB, `🔒` indicator, machine-bound limitation, backward compatibility |
...
| [Terminal: Copy & Paste](user-guide/terminal.md) | Copy selected text, paste commands, right-click menu, keyboard shortcuts, and accessibility |
...
| Document | What it covers |
| -------- | -------------- |
| [PEM Encryption — Technical](technical/pem-encryption.md) | DPAPI encryption, data model, priority/fallback, export/import, validation, testing |

## API

| Document | What it covers |
| -------- | -------------- |
| [Connection API](api/connection-api.md) | `SelectPrivateKeyFile`, `ConvertPPKToPEM`, `Create`/`Update`/`Import` connection, `ConnectSSH` — with encrypted-key params |
...
| [UploadDialog Component](components/upload-dialog.md) | Component reference — props, local state/refs, `sftp:progress` handling, integration, styling, accessibility |
| [FileExplorer Component](components/file-explorer.md) | Component reference — toolbar buttons, Add menu, Create Folder/File dialogs, validation, error handling |
| [Terminal Component](components/terminal.md) | Component reference — copy/paste handlers, Wails clipboard API, xterm.js selection, context menu, a11y |
